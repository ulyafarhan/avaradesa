...
class GeminiProvider implements AiProviderInterface
{
    ...
    public function generateResponse(string $userMessage, string $chatId, ?string $context = null): ?string
    {
        try {
            ...
            $aiResponse = Ai::driver('gemini')->chat($fullPrompt);
            ...
            Log::error('GeminiProvider (laravel/ai) generateResponse Error: ' . $e->getMessage());
            return null;
        }
    }
    ...
    public function fixCopywriting(string $text, ?string $title = null): ?string
    {
        try {
            ...
            return Ai::driver('gemini')->chat($prompt) ?: null;

        } catch (\Exception $e) {
            Log::error('GeminiProvider (laravel/ai) fixCopywriting Error: ' . $e->getMessage());
            return null;
        }
    }
    ...
    public function generateSeoMetadata(string $title, string $content): ?array
    {
        try {
            ...
            $raw = Ai::driver('gemini')->chat($prompt);
            ...
            Log::error('GeminiProvider (laravel/ai) generateSeoMetadata Error: ' . $e->getMessage());
            return null;
        }
    }

    /**
     * Memeriksa ketersediaan layanan AI Gemini.
     *
     * @return bool  true jika driver Gemini tersedia dan merespons
     */
    public function checkHealth(): bool
    {
        try {
            $response = Ai::driver('gemini')->chat('ping');
    ...
    protected function findSemanticCachedResponse(string $userMessage): ?string
    {
        ...
        // Exact match dari log terakhir 24 jam
        ...
        // Semantic match menggunakan Jaccard Similarity
        ...
        foreach ($recentLogs as $log) {
            ...
            // Levenshtein untuk pesan pendek
